...
select luuserid,luusername,extract(year from lfreturndate) month,max(lffineamt) fine_amt from LIBUSERS join LIBFINES using(luuserid) group by(extract(year from lfreturndate),luuserid,luusername);
